...
import '../add_event/event_location.dart';

class EditEvent extends StatefulWidget {
  
  final Map eventDetails;
  const EditEvent({ Key? key, required this.eventDetails }) : super(key: key);
...
class _EditEventState extends State<EditEvent> {
  ...
  void initState() {
    super.initState();
    eventTitle.text = widget.eventDetails['eventTitle'];
    eventDescription.text = widget.eventDetails['eventDescription'];
    eventDate.text = widget.eventDetails['eventDate'];
    eventAddress.text = widget.eventDetails['eventAddress'];
    eventPrice.text = widget.eventDetails['eventPrice'];
    selectedCity = widget.eventDetails['eventCity'];
    //selectedCategory = widget.eventDetails['selectedCategory'];
    latitude = widget.eventDetails['latitude'];
    longitude = widget.eventDetails['longitude'];
  }
  ...
  void editEvent()async{
    ...
    { 
      ...
      if(setEventPrice)
        eventPrice.text = "0";
      dynamic result = await AppController().editEvent(widget.eventDetails, eventTitle.text, eventDescription.text, eventDate.text, selectedCategory!, eventAddress.text, selectedCity!, eventPrice.text, latitude, longitude);
      EasyLoading.dismiss();
      if(result['Status'] == 'Success')
      {
        Get.back(result: true);
        Constants.showDialog('Event has been edited successfully');
      }
      else
      {
