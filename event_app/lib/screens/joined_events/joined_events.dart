...
class JoinedEvents extends StatefulWidget {
  const JoinedEvents({ Key? key }) : super(key: key);
...
class _JoinedEventsState extends State<JoinedEvents> {
  ...
  void getAllJoinedEvents()async{
    allJoinedEvents.clear();
    EasyLoading.show(status: 'Please wait', maskType: EasyLoadingMaskType.black);
    dynamic result = await AppController().getAllJoinedEvents(allJoinedEvents);
    EasyLoading.dismiss();
    if(result['Status'] == 'Success')
    {
     setState(() {
       print(allJoinedEvents.length);
     });
    }
    else
    {
  ...
  void leaveEvent(Map event, int index)async{
    EasyLoading.show(status: 'Please wait', maskType: EasyLoadingMaskType.black);
    dynamic result = await AppController().leaveEvent(event);
    EasyLoading.dismiss();
    if(result['Status'] == 'Success')
    {
     setState(() {
       allJoinedEvents.removeAt(index);
     });
    }
    else
    {
      ...
      body: Container(
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: (allJoinedEvents.isEmpty) ? Container(
                ...
                child: MediaQuery.removePadding(
                  ...
                  child: ListView.builder(
                    itemCount: allJoinedEvents.length,
                    itemBuilder: (_, i) {
                      return eventCell(allJoinedEvents[i], i);
                    },
                    shrinkWrap: true,
                  ),
                ),
